...
static const uint16_t crc16Table[256] PROGMEM = {
...
uint16_t Crc_getCrc16Modbus(uint8_t *data, uint32_t length){
    ...
    while (length--){
		...
		crc = (crc >> 8) ^ pgm_read_word(&crc16Table[(crc & 0xFF) ^ *data++]);
...
uint32_t Crc_getCrc32(uint8_t* data, uint32_t length)
{
    ...
    while (length--){
        ...
        crc = (crc >> 8) ^ pgm_read_dword(&crc32Table[(crc ^ *data++) & 0xFF]);
...
uint16_t Crc_getCrc16Flash (uint32_t length)
{
	...
	while (length--){
		...
		crc = (crc >> 8) ^ pgm_read_word (&crc16Table[(crc & 0xFF) ^ pgm_read_byte_far(counter)]);
		counter++;
	}
	return crc;
}
